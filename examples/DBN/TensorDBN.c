...
int main(int argc, char **argv){
    if(argc != 12){
        fprintf(stderr,"\nUsage: TensorDBN <training set> <testing set> <output results file name> <cross-validation iteration number> \
    ...
    fprintf(stderr,"\nRunning TensorPSO ... ");
    runTensorPSO(s, _QUATERNION_, Bernoulli_BernoulliDBN4Reconstruction, Train, op, n_layers, n_epochs, batch_size, n_gibbs_sampling, eta_bound);

    fprintf(stderr,"\n\nRunning TensorDBN with best parameters on training set ... ");
    ...
    fprintf(stderr,"\n\nRunning TensorDBN for reconstruction on testing set ... ");
