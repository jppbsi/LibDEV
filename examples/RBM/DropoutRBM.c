...
int main(int argc, char **argv){
    if(argc != 11){
        fprintf(stderr,"\nUsage: DropoutRBM <training set> <testing set> <output results file name> <cross-validation iteration number> \
    ...
    fprintf(stderr,"\nRunning PSO ... ");
    runPSO(s, BernoulliRBMWithDropout, Train, op, n_epochs, batch_size, n_gibbs_sampling, eta_bound);
    
    fprintf(stderr,"\n\nRunning Dropout RBM with best parameters on training set ... ");
