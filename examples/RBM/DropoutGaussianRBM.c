...
int main(int argc, char **argv){
    if(argc != 11){
        fprintf(stderr,"\nUsage: DropoutGaussianRBM <training set> <testing set> <output results file name> <cross-validation iteration number> \
    ...
    double *eta_bound, errorTrain, errorTest, p, q, *sigma;
    FILE *f = NULL;
    ...
    fprintf(stderr,"\nRunning PSO ... ");
    runPSO(s, Gaussian_BernoulliRBMWithDropout, Train, n_epochs, batch_size, n_gibbs_sampling, sigma, eta_bound);
    
    fprintf(stderr,"\n\nRunning Dropout Gaussian RBM with best parameters on training set ... ");
    ...
    errorTrain = GaussianBernoulliRBMTrainingbyContrastiveDivergencewithDropout(DatasetTrain, m, n_epochs, n_gibbs_sampling, batch_size, p, q);
    
    fprintf(stderr,"\n\nRunning Dropout Gaussian RBM for classification on testing set ... ");
    errorTest = GaussianBernoulliRBMReconstructionwithDropout(DatasetTest, m, p, q);
    fprintf(stderr,"\nOK\n");
