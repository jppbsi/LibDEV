...
int main(int argc, char **argv)
{
    ...
    s = ReadSearchSpaceFromFile(argv[4], _BHA_);
    ...
    InitializeSearchSpace(s, _BHA_);
    fprintf(stderr, "\nOk\n");

    fflush(stderr);
    fprintf(stderr, "\nRunning BHA ... ");
    gettimeofday(&tic, NULL);
    runBHA(s, FeatureSelectionOPF, Train, Evaluate, optTransfer);
    ...
    fprintf(stderr, "\nTraining set\n");
    WriteSubgraph(newTrain, "training.bha.dat");
    fprintf(stderr, "\n\nTesting set\n");
    WriteSubgraph(newTest, "testing.bha.dat");
    fflush(stderr);
    fprintf(stderr, "\nOK\n");
