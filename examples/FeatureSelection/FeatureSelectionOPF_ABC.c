...
int main(int argc, char **argv)
{
    ...
    s = ReadSearchSpaceFromFile(argv[4], _ABC_);
    ...
    InitializeSearchSpace(s, _ABC_);
    fprintf(stderr, "\nOk\n");

    fflush(stderr);
    fprintf(stderr, "\nRunning ABC ... ");
    gettimeofday(&tic, NULL);
    runABC(s, FeatureSelectionOPF, Train, Evaluate, optTransfer);
    ...
    fprintf(stderr, "\nTraining set\n");
    WriteSubgraph(newTrain, "training.abc.dat");
    fprintf(stderr, "\n\nTesting set\n");
    WriteSubgraph(newTest, "testing.abc.dat");
    fflush(stderr);
    fprintf(stderr, "\nOK\n");
