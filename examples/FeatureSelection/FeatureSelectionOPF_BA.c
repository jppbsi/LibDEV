...
int main(int argc, char **argv)
{
    ...
    s = ReadSearchSpaceFromFile(argv[4], _BA_);
    ...
    InitializeSearchSpace(s, _BA_);
    fprintf(stderr, "\nOk\n");

    fflush(stderr);
    fprintf(stderr, "\nRunning BA ... ");
    gettimeofday(&tic, NULL);
    runBA(s, FeatureSelectionOPF, Train, Evaluate, optTransfer);
    ...
    fprintf(stderr, "\nTraining set\n");
    WriteSubgraph(newTrain, "training.ba.dat");
    fprintf(stderr, "\n\nTesting set\n");
    WriteSubgraph(newTest, "testing.ba.dat");
    fflush(stderr);
    fprintf(stderr, "\nOK\n");
