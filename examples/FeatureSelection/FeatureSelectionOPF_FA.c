...
int main(int argc, char **argv)
{
    ...
    s = ReadSearchSpaceFromFile(argv[4], _FA_);
    ...
    InitializeSearchSpace(s, _FA_);
    fprintf(stderr, "\nOk\n");

    fflush(stderr);
    fprintf(stderr, "\nRunning FA ... ");
    gettimeofday(&tic, NULL);
    runFA(s, FeatureSelectionOPF, Train, Evaluate, optTransfer);
    ...
    fprintf(stderr, "\nTraining set\n");
    WriteSubgraph(newTrain, "training.fa.dat");
    fprintf(stderr, "\n\nTesting set\n");
    WriteSubgraph(newTest, "testing.fa.dat");
    fflush(stderr);
    fprintf(stderr, "\nOK\n");
